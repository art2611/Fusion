    ...
    if args.dataset == "regdb":
        for trial in range(10):
            ...
            if args.reid == "BtoB" :
                ...
                gall_loader = torch.utils.data.DataLoader(gallset, batch_size=test_batch_size/2, shuffle=False,
                                                          num_workers=workers)
                nquery = len(query_label)
                ngall = len(gall_label)

                queryset = TestData_both(query_img, query_img_t, query_label, transform=transform_test, img_size=(img_w, img_h))
                query_loader = torch.utils.data.DataLoader(queryset, batch_size=test_batch_size/2, shuffle=False,
                                                           num_workers=4)
